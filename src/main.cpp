...
#define cmPerLightbarPixel  4          // Must be a multiple of cmPerDistInt
...
// Distance from line or cross track error
...
// Self-test routine delay between steps
uint32_t self_delay = 1500;
...
void lightbar(uint8_t distanceFromLine ){
  ...
  { 
    ...
    // Small self-test routine to check lights / LCD providing feedback and BT name to user
    M5.Lcd.print("Starting setup ...");
    delay(self_delay);
    lightbar(254);
    delay(self_delay);
    lightbar(0);
    ...
    #endif
    M5.Lcd.println("Waiting on AOG data");
    M5.Lcd.setCursor(0, 110);
    M5.Lcd.println("Bat Cap " + String(getBatCapacity()) + "%");
